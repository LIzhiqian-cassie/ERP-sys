  ...
  if(isset($_POST["confirminbound"])) {
    ...
    $inboundcost = $_POST["inbound_cost"];
      ...
      // {
        ...
        // for($i=0; $i < $N; $i++)
        // {
          $sql = "INSERT INTO `inbound_items_list` (`inbound_id`, `purchasing_id`) VALUES ('$inbound_id', '$purchase_id')";
          $result = $conn->query($sql);

          $sql_status = "UPDATE purchase SET inbound_status = '1' WHERE purchasing_id= $purchase_id";
          $result = $conn->query($sql_status);

          $sql2 = "SELECT product_id, quantity FROM purchase_items_list where purchasing_id='$purchase_id'";
        ...
        // }
      // }
      header("location: purchase-info?id=$purchase_id");
    }
?>